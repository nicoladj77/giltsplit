<?php
	$hercules_font_array = array();
	$hercules_char_array = array();
...
function of_recognized_font_weights() {
	$default = array(
		'100'      => __('Ultra-Light 100', 'options_framework_theme'),
		'200'      => __('Light 200', 'options_framework_theme'),
		'300'        => __('Book 300', 'options_framework_theme'),
		'400'      => __('Normal 400', 'options_framework_theme'),
		'500'      => __('Medium 500', 'options_framework_theme'),
		'600'        => __('Semi-Bold 600', 'options_framework_theme'),
		'700'      => __('Bold 700', 'options_framework_theme'),
		'800'      => __('Extra-Bold 800', 'options_framework_theme'),
		'900'        => __('Ultra-Bold 900', 'options_framework_theme')
		);
...
if ( !function_exists( 'options_typography_google_fonts' ) ) {
	function options_typography_google_fonts() {
		global $hercules_font_array, $hercules_char_array;
		...
		$logo_typography = of_get_option('logo_typography', 'Arial');
		...
		if (isset($hercules_font_array)) {
			foreach ($hercules_font_array as $key => $value) {
				options_typography_enqueue_google_fonts($hercules_font_array[$key], $hercules_char_array[$key]);
			}
		}        
	}
}
...
function options_typography_enqueue_google_font($key, $font) {
	...
	//if ( $font == 'Raleway' )
		//$font = 'Raleway:100,200,300,400,500,600,700,800,900';
	//$font = str_replace(" ", "+", $font);
	//$hercules_font_array[$key] = $font;
	//if ( $font == 'Open Sans' )
...
if( $font == 'Raleway' ):
	...
	$font = str_replace(" ", "+", $font);
	$hercules_font_array[$key] = $font;
elseif( $font == 'Open Sans' ): // Note the combination of the words.
    $font = 'Open Sans:300,400,600,700,800';
	$font = str_replace(" ", "+", $font);
	$hercules_font_array[$key] = $font;
elseif( $font == 'Lato' ): // Note the combination of the words.
    $font = 'Lato:100,300,400,700,900';
	$font = str_replace(" ", "+", $font);
	$hercules_font_array[$key] = $font;
elseif( $font == 'Merriweather' ): // Note the combination of the words.
    $font = 'Merriweather:300,400,700,900';
	$font = str_replace(" ", "+", $font);
	$hercules_font_array[$key] = $font;
elseif( $font == 'Droid Sans' ): // Note the combination of the words.
    $font = 'Droid Sans:400,700';
	$font = str_replace(" ", "+", $font);
	$hercules_font_array[$key] = $font;
else:
    $font = str_replace(" ", "+", $font);
...
function options_typography_enqueue_google_character_set($key, $character) {
	global $hercules_char_array;

	$hercules_char_array[$key] = '&amp;subset=' . $character;
}
...
function options_typography_font_styles($option, $selectors) {
		$output = $selectors . ' { ';
		$output .= 'font-weight: ' . $option['weight'] . '; font-style: ' . $option['style'] . '; font-size: ' . $option['size'] . '; line-height: '.$option['lineheight'].'; font-family: ' . $option['face'] . '; ';
