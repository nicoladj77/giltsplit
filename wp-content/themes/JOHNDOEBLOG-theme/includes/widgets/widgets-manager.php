...
// get theme name for theme
function get_theme_name() {

	$theme_name = HS_CURRENT_THEME;
...
add_filter( 'widget_update_callback', 'theme_widget_update', 10, 4);

// definition of the additional operation
function theme_widget_update($instance, $new_instance, $old_instance, $widget) {
...
add_filter( 'dynamic_sidebar_params', 'theme_dynamic_sidebar_params' );
function theme_dynamic_sidebar_params( $params ) {
...
add_action( 'sidebar_admin_setup', 'theme_add_widget_control');
function theme_add_widget_control() {
	global $wp_registered_widgets; 
	...
	foreach ( $wp_registered_widgets as $id => $widget ) {
		if ( isset($wp_registered_widget_controls[$id]) ) {
			...
			$wp_registered_widget_controls[$id]['callback'] = 'theme_widget_control';
		}
	}
}

// function to add the widget control 
function theme_widget_control() {
	...
	theme_widget_control_styles_list($params[0]['widget_id'], $id, $responsiveMode, $usersMode);
}

function theme_widget_control_styles_list($widget_name, $id, $value2, $value3) {
	$themename = get_theme_name();
...
function check_widget_visibility($id) {
	...
	if ( isset($options[$id]) && $options[$id] != '' ) {
		// create function
		$conditional_function = create_function('', 'return '.theme_condition($options_type[$id], $options[$id], $users[$id]).';');
	...
	$wp_registered_widgets[$id]['themestate'] = $conditional_result;
 ...
 *
 **/
function theme_condition($mode, $input, $users) {
	// Example input:
